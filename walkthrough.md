...
- **Logo Integration**: Replaced the navbar logomark with the full vector SVG and logotype (supporting automatic dark mode color switching via `currentColor`).
...
- **Hero Title Refinement**: Adjusted the hero title CSS clamp and added the `whitespace-nowrap` class to keep "Monarch's Dining" on a single line across viewports.
